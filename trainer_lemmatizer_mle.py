...
class TrainerLemmatizerMLE:
  def __init__(self,model,loader,args):
    self.args = args
    self.loader = loader # data loader
    self.n_classes = loader.get_vocab_size()
    self.model = model
    ...
    self.stop_id = loader.vocab_oplabel.get_label_id(STOP_LABEL)
    self.pad_id = loader.vocab_oplabel.get_label_id(PAD_TOKEN)
  ...
  def repeat_horizontal(self,h,x):
    ...
    if isinstance(h, torch.Tensor):
      ...
      return tuple(self.repeat_horizontal(v,x) for v in h)

  def repeat_hidden(self,h,x):
    # x: num horizontal tiles (row), 
    if isinstance(h, torch.Tensor):
      ld,_,hid_size = h.shape
      return h.repeat(1,1,x).view(ld,-1,hid_size)
    else:
      return tuple(self.repeat_hidden(v,x) for v in h)
  ...
  def compute_loss(self,pred_w,gold_w,debug=0):
    ...
    mask = (gold_w!=self.pad_id).float() # [bs,W]
    sum_mask = mask.sum(1)
      ...
      for i,sent in enumerate(predicted):
        ...
        for j in range(len_sent):
          ...
          optokens = [data_vocabs.vocab_oplabel.get_label_name(x) \
                        for x in w_op_seq if x!=self.pad_id]
